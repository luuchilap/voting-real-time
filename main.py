        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if 'results' in data and len(data['results']) > 0:
                    ...
                    print(f"Attempt {attempt + 1}: No results in API response, retrying...")
                    time.sleep(1)
            else:
                print(f"Attempt {attempt + 1}: HTTP {response.status_code}, retrying...")
                time.sleep(1)
        except Exception as e:
            print(f"Attempt {attempt + 1}: Error fetching data: {e}, retrying...")
            time.sleep(1)
    raise Exception("Failed to generate voter data after multiple attempts")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if 'results' in data and len(data['results']) > 0:
                    ...
                    print(f"Attempt {attempt + 1}: No results in API response for candidate, retrying...")
                    time.sleep(1)
            else:
                print(f"Attempt {attempt + 1}: HTTP {response.status_code} for candidate, retrying...")
                time.sleep(1)
        except Exception as e:
            print(f"Attempt {attempt + 1}: Error fetching candidate data: {e}, retrying...")
            time.sleep(1)
    raise Exception("Failed to generate candidate data after multiple attempts")
