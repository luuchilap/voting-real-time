...
if __name__ == "__main__":
    ...
    postgres_jar_path = os.path.join(project_root, "postgresql-42.7.1.jar")
    checkpoint1_path = os.path.join(project_root, "checkpoints", "checkpoint1")
    checkpoint2_path = os.path.join(project_root, "checkpoints", "checkpoint2")
    
    # Initialize SparkSession
    # Note: Using packages with explicit exclusions to resolve Scala version conflicts
    spark = (SparkSession.builder
             .appName("ElectionAnalysis")
             .master("local[*]")  # Use local Spark execution with all available cores
             .config("spark.jars.packages",
                     "org.apache.spark:spark-sql-kafka-0-10_2.12:3.5.0")  # Spark-Kafka integration (using 2.12 to match PySpark's Scala version)
             .config("spark.jars",
                     postgres_jar_path)  # PostgreSQL driver
             .config("spark.sql.adaptive.enabled", "false")  # Disable adaptive query execution
             .config("spark.sql.streaming.kafka.useDeprecatedOffsetFetching", "false")
             .config("spark.sql.streaming.checkpointLocation", os.path.join(project_root, "checkpoints"))
             .config("spark.driver.extraJavaOptions", 
                     "-Dio.netty.tryReflectionSetAccessible=true -XX:+UseG1GC")
    ...
    # Wait for either stream to terminate (both should run indefinitely)
