...
def update_data():
    ...
    if not data:
        st.warning("No aggregated vote data available yet. Please wait for Spark Streaming to process votes.")
        return
    ...
    if len(results) == 0:
        st.warning("No vote results available yet.")
        return
    ...
    if location_data:
        ...
        if len(location_result) > 0:
            ...
            st.info("Location data will appear here once Spark Streaming processes the votes.")
    else:
        st.info("Location data will appear here once Spark Streaming processes the votes.")
